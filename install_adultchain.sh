...
# or run with sudo
# 2) chmod +x install_masternode.sh
# 3) run ./install_masternode.sh
# or sudo ./install_masternode.sh
...
if [ -f $homedir/.$datadir/$datadir.conf ]; then
	...
	echo -e masternodeaddr= >> ~/.$datadir/$datadir.conf & wait $!
...
clear
	...
	echo -e ""
	sleep 2
	cat $homedir/.$datadir/debug.log | grep CActiveMasternode::EnableHotColdMasterNode
	sleep 2
...
echo -e ""
echo -e "This means the masternode is Enabled"
echo -e ""
echo -e ""
}
